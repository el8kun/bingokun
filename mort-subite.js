...
function roomRowToApp(row) {
  ...
  return {
    ...
    preset: String(row.preset || "").replace("mort-subite:", "") || "",
...
function updatePresetHelp() {
  ...
  const labels = {
    "global-easy": "Mort Subite facile : stars, grands clubs et catégories simples.",
    "global-normal": "Mort Subite équilibrée : joueurs connus, moins de profils obscurs.",
    "global-hard": "Mort Subite difficile : base plus large.",
    "ligue1": "Mort Subite Ligue 1.",
...
createRoomBtn?.addEventListener("click", async () => {
  try {
    ...
    if (createRoomBtn) {
      ...
      createRoomBtn.textContent = "Créer une Mort Subite";
    }
  }
});
...
async function createRoom() {
  ...
  const roomRow = {
    ...
    status: "waiting",
    host_uid: uid,
    preset: `mort-subite:${selectedPreset}`,
    preset_label: `${SUDDEN_DEATH_MODE_LABEL} — ${presetModeSelect?.selectedOptions?.[0]?.textContent || selectedPreset}`,
    grid,
    deck,
...
function renderWaitingRoom() {
  ...
  waitingHostHint.textContent = isHost
    ? "Tu es le créateur de la Mort Subite. Lance la partie quand tout le monde est là."
    : "En attente du créateur de la room Mort Subite.";
...
function renderLeaderboard(players = participantsData) {
  ...
  leaderboardEl.innerHTML = economyNotice + sorted.map((player, index) => {
    ...
    const score = isFinished ? (player.finalScore || 0) : calculateBoardScore(player.board || {}, roomData?.grid || []);
...
async function advanceMyPlayer(manual = false) {
  ...
  await runPlayerAction(async () => {
    ...
    if (currentIndex >= deckLength) {
      await finishParticipant("sudden_death_clear");
      return;
    }

    if (manual && getSkipStreak(board) >= SUDDEN_DEATH_MAX_CONSECUTIVE_SKIPS) {
...
async function ensureResultSaved(board, finalScore, bingos) {
  ...
  try {
    await supabaseInsertResult({
      ...
      points,
      month_key: monthKey,
      mode_label: roomData.presetLabel || `Mort Subite — ${roomData.preset || selectedPreset || "mode libre"}`,
      created_at: nowIso()
    });
...
function updateFinishOverlay(finalScore, bingoCount, wrongCount, accuracy, rank, totalPlayers, scoreMax = 30, validCells = 0) {
  ...
  let title = "Mort Subite terminée";
  let subtitle = getFinishMessage(myData?.finishReason);

  if (false && finalScore >= 25) {
    title = 'Masterclass !';
    subtitle = 'Énorme performance sur cette grille.';
  } else if (false && finalScore >= 20) {
    title = 'Très solide !';
    subtitle = 'Belle partie, ta grille tient bien la route.';
  } else if (false && finalScore <= 12) {
    title = 'À retenter !';
